      ...
      <?php
        ...
        foreach($tv_id_similar->results as $sim){
          ...
          $poster 	= $sim->poster_path;
			if (empty($backdrop) || is_null($backdrop)||empty($poster) || is_null($poster)){
        $output.='<li><a href="tvshow.php?id='.$sim->id.'"><img src="'.dirname($_SERVER['PHP_SELF']).'/image/no-gambar.jpg'.'"><h5>'.$sim->original_name.'</h5></a></li>';
        
       
			} else {
        

        $output.='<li><a href="tvshow.php?id='.$sim->id.'"><img src="http://image.tmdb.org/t/p/w300'.$sim->backdrop_path.'"><h5>'.$sim->name.'</h5></a></li>';
